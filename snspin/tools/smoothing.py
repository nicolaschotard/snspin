...
def prediction_error(r,B,var, W=None,verbose=False):
    ...
    if W == None :
        ...
        if pe < 0 and verbose:
            print "WARNING <prediction_error>: pe < 0, variance probably under estimated"
    return pe
#        return dot(dot(r,W),r) + 2 * sum(dot(W,B) * diag(var)) - len(var)
#+2*sum(diag(B)*e)
...
def spline_tck_prediction_error(x,y,var,tck,dbg=False,Wcorr=None):
    ...
    r=out-y
    # here by construct, the spline uses only the diagonal term on W for minimization : this is NOT the Wcorr ...
    W=N.diag(1./var)
...
def spline_find_s(x,data,var,s0=None,corr=0.0,dbg=False):
    ...
    if corr != 0.0:
        ...
        func=lambda s,x,d,v : spline_prediction_error(x,d,v, (s+N.sqrt(s**2+s_tweak))/2.,dbg=dbg,W=W)[2]
    else:
        func=lambda s,x,d,v : spline_prediction_error(x,d,v, (s+N.sqrt(s**2+s_tweak))/2.,dbg=dbg)[2]
...
def secondpar(par,e):
    ...
    c=par[0]
    delta=discriminant(par)
    dAdA = - (e * b**4 - 6 * e * b**2 * delta - 3 * e * delta**2 + 8 * delta**(1.5) * b ) / ( 8 * delta**1.5 * a**3)
    dBdB = - e * c / delta**1.5
    dCdC = - e * a / ( 2 * delta**1.5)
    dAdB = ( e * b**3 - 3 * e * b * delta + 2 * delta**1.5 ) / ( 2 * delta**1.5 * a**2)
    dAdC = -e * c / delta**1.5
    dBdC = e * b / (delta**1.5)
    # to keep the order of the parameters
...
def B_matrix_gauss_filter(spec,sigma):
    ...
    for i in spec.index_list:
        B[:,i]=1./(spec.x*N.sqrt(2*N.pi)*sigma) * N.exp( -( (N.log(spec.x)-N.log(spec.x[i]))/sigma)**2/2.) * spec.step    
        B[:,i]/=N.sum(B[:,i])
    return B
    ...
    while converged == 0 and ierr==0 :
        if fs[1]==N.min(fs):
            ...
            if xs[2]<ubound:
                ubound=xs[2]
        iter += 1
        m=N.array([[xs[0]**2,xs[0],1],[xs[1]**2,xs[1],1],[xs[2]**2,xs[2],1]])
...
def sg_find_num_points(x,data,var,pol_degree=2,corr=0.0,verbose=False):
    ...
    toler=N.max([e[n(i_iteration/4)]-e[n(i_iteration/2)] , e[min(len(x),n(i_iteration))]-e[n(i_iteration/2)]])/2
    ...
    for num_points in N.arange( n(i_iteration/4), min( n(i_iteration), len(x) ), max([1,n(i_iteration/4)/10]) ):
        B=B_matrix_sg(num_points,pol_degree,len(x))
        yy=N.dot(B,data)
        pe=prediction_error(yy-data,B,var,W)
        e[num_points]=pe
...
def comp_epsilon_terms(x,y,v,p,corr=0,mode='sp',all=False):
    ...
    TB    = N.trace(B)
    lv    = len(v)
    if all: return rchi2,TB,lv,p,yy
    else: return rchi2,TB,lv,p
