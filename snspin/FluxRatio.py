...
def hubble_fit_fratio(idr, phase=0, window=2.5, k=10,
    ...
    #Get the uncorrected Hubble residual from the IDR and the SALT2 measurement
    ...
    Hdata = Hubblefit.HubbleData(z, dz, m, dm, sne=sne, run=True, verbose=False)
    ...
    for i in range(len(X[0])):
        ...
        for j in range(len(X[0])):
            ...
            filt = N.prod(map(lambda x: x==x,[f,vf,z,dz,m,dm]),
                          axis=0,dtype=bool)
            f,vf,zp,dzp,mp,dmp = map(lambda x: x[filt], [f,vf,z,dz,m,dm])
...
def add_GP_data(idr, hfk_dir=None):
    ...
    files = glob.glob(hfk_dir + (hfk_dir.endswith('/') and '' or '/') + '*.dat')
