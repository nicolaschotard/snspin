   ...
   """
    ...
    def _get_metrics_name(self):
        ...
        for i in range(len(self.listSNe)):
            try:
                list = self.pdata[self.listSNe[i]]['spectra'][
                    (self.pdata[self.listSNe[i]]['spectra'].items())[0][0]].keys()
                break
            except:
                continue
    ...
    def _organize_parameters(self):
        ...
        for SN in self.listSNe:
            ...
            if self.hdata is not None:
                ...
                psn['salt2.RestFrameMag_0_B.err'] = hsn[
                    'salt2.RestFrameMag_0_B.err']
            ...
            if not len(spectra) == 1:
                ...
                phases = N.array([psn['spectra'][spec]['obs.phase']
                                  for spec in spectra])
        ...
        for SN, spectrum in zip(self.listSNe, self.spectra_at_max):
            ...
            for metric in self.metrics_name:
                ...
                metrics[metric].append(float(tdic[spectrum]['phrenology.' +
                                                            metric]))
                metrics[
                    metric + '.err'].append(float(tdic[spectrum]['phrenology.' + metric + '.err']))
    ...
    def _define_filter_colorbar(self):
        ...
        if self.cutkey is not None:
            self.filter = self._cutvalues(
                self.cutkey, inf=self.inf, sup=self.sup)
        else:
    ...
    def plot_parameters(self, key1=None, key2=None, linfit=True,
        ...
        if key1 is not None and key2 is not None:
            ...
            print "Give me one (key1) or two keys"
            return
        ...
        if self.keepcut and (~self.filter).any():
            ...
            if self.text_sne:
                for xx, yy, sn in zip(self.x[0][~self.filter][filt2],
                    ...
                    if sn[:3] == 'SNF':
                        ax.text(xx, yy, ' %s' %
                                sn[5:], rotation=45, size='x-small')
                    else:
                        ax.text(xx, yy, ' %s' %
                                sn[4:], rotation=45, size='x-small')
    ...
    def multiplots(self,  list_keys=[], list_keys2=False, space=0.04, left=0.08,
            ...
            for j, key2 in enumerate(list_keys2):
                ...
                if colorlegend and not corrcol:
                    ...
                    if corrcol:
                        ...
                        col = 'k'
                    ax.plot(y[0][filter][mask], x[0][
                            filter][mask], '.', color=col)
                    if errorbar:
        ...
        if colorlegend:
            ...
            cb = fig.colorbar(scat, format='%.1f')
            # Legend of the colorbar
            if colorlegend == 'corr':
